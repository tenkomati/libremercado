...
- 2026-04-23: se agrego operación admin para seguros embebidos sin proveedor real: `GET /insurance/policies`, `GET /insurance/policies/:id`, `PATCH /insurance/policies/:id/status`, listado dentro de `/admin`, ficha `/admin/insurance/:id` y cambio manual de estado/URL de póliza para QA y sandbox interno.
- 2026-04-24: se agrego flujo manual de reclamo de seguro sin proveedor real: `POST /insurance/policies/:id/claim`, formulario en `/account`, persistencia del claim en `rawPayload.claim`, transición a `CLAIMED`, auditoría y notificaciones para comprador/vendedor.
- 2026-04-24: se cerró el circuito beta del claim: evidencias por URL en el reclamo, visualización en `/account` y `/admin/insurance/:id`, y resolución operativa manual vía `PATCH /insurance/policies/:id/claim/resolve`.
